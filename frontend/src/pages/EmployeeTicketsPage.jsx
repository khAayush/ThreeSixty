...
const EmployeeTicketsPage = ({ onLogout }) => {
  ...
  const fetchTickets = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${API_URL}/tickets?userId=${currentUser._id}`);
  ...
  const handleCreateTicket = async (formData) => {
    const toastId = showLoading("Creating ticket...");
    try {
      ...
      showSuccess("Ticket created successfully!", toastId);
      setIsModalOpen(false);
    } catch (err) {
      showError("Failed to create ticket", toastId);
  ...
  const handleCloseTicket = async () => {
    ...
    setClosing(true);
    const toastId = showLoading("Closing ticket...");
    try {
      const res = await fetch(`${API_URL}/tickets/${selectedTicket._id}/close`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          note: closeNote,
      ...
      setCloseNote("");
      showSuccess("Ticket closed successfully!", toastId);
    } catch (err) {
      showError("Failed to close ticket", toastId);
      ...
      <div className="flex flex-col h-[calc(100vh-4rem)] p-4 md:p-8 max-w-7xl mx-auto w-full">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 shrink-0 gap-4">
          <div>
            ...
            <p className="text-slate-500 text-sm">View and track the status of your requests</p>
          </div>
          <button onClick={() => setIsModalOpen(true)} className="flex items-center justify-center gap-2 px-5 py-2.5 bg-brand text-white rounded-xl text-sm font-bold hover:brightness-110 shadow-md shadow-brand/20 transition-all w-full md:w-auto">
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left p-4 rounded-xl border transition-all duration-200 flex gap-3 items-start ${
                          isActive ? "border-brand bg-brand/5 shadow-sm" : "border-slate-200 bg-white hover:border-brand/40 hover:shadow-sm"
                        }`}
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <h4 className="font-semibold text-slate-800 mb-1 truncate">{ticket.title}</h4>
                          <p className="text-sm text-slate-500 mb-3 truncate">{ticket.userName}</p>
                          <div className="flex justify-between items-center gap-2">
                            <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider shrink-0 ${getStatusBadge(ticket.status)}`}>
                              {ticket.status}
                            </span>
            ...
            {selectedTicket ? (
              <>
                <div className="lg:hidden p-4 border-b border-slate-100 shrink-0">
                  <button onClick={() => setShowDetailOnMobile(false)} className="flex items-center space-x-2 text-slate-500 hover:text-brand text-sm font-semibold">
                ...
                <div className="p-6 md:p-8">
                  <div className="flex justify-between items-start mb-6">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        <h3 className="font-bold text-slate-800 text-lg leading-tight">{selectedTicket.userName}</h3>
                        <p className="text-slate-400 text-sm mt-0.5">
                          Created: {formatDate(selectedTicket.createdAt)}
                        </p>
                      </div>
                    </div>
                    <span className={`px-4 py-1.5 rounded-lg text-sm font-semibold tracking-wide capitalize shrink-0 ${
                      selectedTicket.status === "Open" ? "bg-amber-50 text-amber-500" : "bg-emerald-50 text-emerald-500"
                    }`}>
                      {selectedTicket.status}
                    </span>
                  </div>

                  <h2 className="text-xl md:text-2xl font-bold text-slate-800">{selectedTicket.title}</h2>
                    ...
                    <div className="flex flex-col">
                      <div className="p-6 md:p-8">
                        ...
                        <p className="text-slate-500 leading-relaxed">
                          {selectedTicket.note || "No note was provided."}
                        </p>
                      </div>
                      ...
                      {selectedTicket.resolvedDate && (
                        <div className="px-6 md:px-8 py-4 bg-slate-50 border-t border-slate-100 flex flex-col sm:flex-row justify-between text-sm text-slate-500 font-medium gap-2">
                          <span>
                            <strong className="text-slate-700">Closed on:</strong> {formatDate(selectedTicket.resolvedDate)}
                          </span>
                          <span>
                            <strong className="text-slate-700">Closed By:</strong> {selectedTicket.resolvedBy || selectedTicket.closedBy || "You"}
                          </span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="border-t border-slate-100">
                      <div className="p-6 md:p-8 bg-slate-50/30">
                        ...
                        <textarea
                          ...
                          className="w-full bg-slate-100 border-none rounded-xl p-4 text-slate-700 focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all resize-none min-h-20 shadow-inner"
                        />
                        <div className="mt-4 flex justify-start">
                          <button
                            onClick={handleCloseTicket}
                            disabled={closing || !closeNote.trim() }
                            className="flex items-center space-x-2 px-6 py-2.5 rounded-xl font-bold transition-all shadow-sm bg-red-500 text-white hover:bg-red-600 hover:shadow-blue-200 disabled:opacity-60 disabled:cursor-not-allowed"
                          >
                            <span>{closing ? "Closing..." : "Close Ticket"}</span>
                          </button>
                        </div>
                      </div>
                    </div>
              ...
              <div className="flex-1 flex items-center justify-center text-slate-400 p-8">Select a ticket to view details</div>
            )}
          </div>
        </div>
      </div>
